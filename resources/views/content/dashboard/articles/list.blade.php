...
@section('content')
<h4 class="py-3 mb-4" dir="{{ app()->isLocale('ar') ? 'rtl' : '' }}"><span class="text-muted fw-light">{{ __('Pages') }} /</span> {{ __('Articles') }}
</h4>

<!-- Responsive Table -->
<div class="card row" dir="{{ app()->isLocale('ar') ? 'rtl' : '' }}">
  <h5 class="card-header">{{__('Articles')}}</h5>
  <div class="row">
    ...
    <a href="{{ route('article.create.index') }}" class="btn btn-icon btn-outline-primary mb-3 mx-1">
      <span class="tf-icons mdi mdi-plus-outline"></span>
    </a>
  </div>

  <div class="table-responsive text-nowrap">
    <table class="table table-striped w-100" id="articles" data-page-length='100'>
      <thead>
...
<script type="text/javascript">
  ...
  var lang = "{{ app()->getLocale() }}";
      ...
      function demoArticle(id) {
        window.open("{{ url('view/articles') }}/" + id, "_blank");
      }

      function deleteArticle(id) {
        Swal.fire({
            title: __("Do you really want to delete this Article?",lang),
            icon: 'warning',
      ...
      function showContextMenu(id, x, y) {
        var contextMenu = $('<ul class="context-menu" dir="{{ app()->isLocale("ar") ? "rtl" : "" }}"></ul>')
            .append('<li><a onclick="editArticle(' + id + ')"><i class="tf-icons mdi mdi-pencil-outline {{ app()->isLocale("ar") ? "ms-1" : "me-1" }}"></i>{{ __("Edit") }}</a></li>')
            .append('<li><a onclick="demoArticle(' + id + ')"><i class="tf-icons mdi mdi-arrow-right-top {{ app()->isLocale("ar") ? "ms-1" : "me-1" }}"></i>{{ __("Demo") }}</a></li>')
            .append('<li class="px-0 pe-none"><div class="divider border-top my-0"></div></li>')
            .append('<li><a onclick="deleteArticle(' + id + ')"><i class="tf-icons mdi mdi-trash-can-outline {{ app()->isLocale("ar") ? "ms-1" : "me-1" }}"></i>{{ __("Delete") }}</a></li>');
