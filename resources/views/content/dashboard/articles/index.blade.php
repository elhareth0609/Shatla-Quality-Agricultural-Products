...
@section('content')
<h4 class="py-3 mb-4" dir="{{ app()->isLocale('ar') ? 'rtl' : '' }}"><span class="text-muted fw-light">{{ __('Pages') }} /</span> {{ $article->title }}
</h4>
...
<div class="row">
  <div class="card mb-4 pb-4">
    ...
    <form id="contentForm" method="POST" action="{{ route('article.update') }}" enctype="multipart/form-data">
      <div class="card-body">
          ...
          <div class="tab-pane fade show active" id="navs-justified-home" role="tabpanel">
            ...
            <div class="card-body">
              <div class="d-flex align-items-start align-items-sm-center gap-4">
                <img src="{{ $article->photoUrl() }}" alt="user-avatar" class="d-block rounded" id="uploadedAvatar" style="width: 340px!important;height: 180px!important"/>
            ...
            <div class="row d-flex">
              ...
              <div class="col-md-6 mb-3">
                <div class="form-floating form-floating-outline">
                  <input type="text" class="form-control" id="address" name="title" placeholder="{{ __('Title') }}" value="{{ $article->title }}" />
                  ...
                  <select id="type" name="type" class="form-select">
                    @foreach ($types as $type)
                      @if ($type->id === $article->type->id)
                        <option value="{{ $type->id }}" selected>{{ $type->getName() }}</option>
                      @else
              ...
              <div class="col-md-6 mb-3">
                <div class="input-group">
                  ...
                  <textarea class="form-control" id="tags" name="tags" aria-label="With textarea">{{ is_array($article->tags) ? implode(', ', $article->tags) : htmlspecialchars($article->tags) }}</textarea>
                </textarea>
                </div>
              </div>
                ...
                <div class="form-floating form-floating-outline">
                  <select id="disease" name="disease" class="form-select">
                    @foreach ($diseases as $disease)
                      @if ($disease->id === $article->disease->id)
                        <option value="{{ $disease->id }}" selected>{{ $disease->getName() }}</option>
                      @else
                        <option value="{{ $disease->id }}">{{ $disease->getName() }}</option>
                      @endif
                    @endforeach
                  </select>
                  <label for="disease">{{ __('Disease') }}</label>
                </div>
              </div>
            </div>
          </div>
            ...
            <div class="photos-container">
              @foreach ($article->photos as $photo)
                <div class="image-container text-center my-w-fit-content mb-2 mx-auto position-relative">
                    ...
                    <img src="{{ $photo->photoUrl() }}" alt="{{ $photo->id }}" class="rounded article-photo" />
                </div>
              @endforeach
            </div>
          </div>
