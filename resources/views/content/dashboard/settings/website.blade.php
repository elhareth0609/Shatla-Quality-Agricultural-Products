...
@section('title', __('Settings - Website'))

@section('content')
<h4 class="py-3 mb-4">
  <span class="text-muted fw-light">{{ __('Settings') }} /</span> {{ __('Website') }}
</h4>
...
<div class="row">
  <div class="col-md-12">
    <ul class="nav nav-pills flex-column flex-md-row mb-4 gap-2 gap-lg-0">
      <li class="nav-item"><a class="nav-link" href="{{ route('settings.account') }}"><i class="mdi mdi-account-outline mdi-20px me-1"></i>{{ __('Account') }}</a></li>
      @if (Auth::user()->isCan('manage website'))
        <li class="nav-item"><a class="nav-link active" href="javascript:void(0);"><i class="mdi mdi-home-outline mdi-20px me-1"></i>{{ __('Website') }}</a></li>
      @endif
    ...
    <form class="card mb-4" id="updateForm" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
      @csrf
      <h4 class="card-header">{{ __('Website Details') }}</h4>
      <!-- Account -->
      <div class="card-body">
